...
class Rapidhiv_Model extends CI_Model {
...
function convetResultsetToArray($Q) {
	...
	function getRapidHivTable() {
        ...
        $aColumns = array('rapidHiv_id', 'year', 'cycle', 'userName', 'parameter', 'results', 'status', 'target', 'timeStamp');

        //this array result column database
        $aResultColumns = array('rapidHiv_id','year', 'cycle', 'parameter', 'userName', 'results', 'status', 'target', 'timeStamp');

        /* Indexed column (used for fast and accurate table cardinality) */
        $sIndexColumn = "rapidHiv_id";
	...
	";
	  ...
	  ";
        ...
        for ($i = 0; $i < count($rResult); $i++) {
            $row = array();
            $row = $rResult[$i];
            $row['insights'] = "<a class='various' data-fancybox-type='iframe' href='" . base_url() . "index.php/rapidHiv/rapidHiv_graph/{$row['year']}'><i class='zoom fa fa-line-chart'></i>{$row['year']}<i class='zoom fa fa-pie-chart'></a>";
            $row['edit'] = "<a class='various' data-fancybox-type='iframe' href='" . base_url() . "index.php/rapidHiv/popupadd/{$row['rapidHiv_id']}'>edit</a>";
            $row['delete'] = "<a class='deleteRecord' onclick= 'deleteRecord({$row['rapidHiv_id']})' >delete</a>";
            $output['aaData'][] = $row;
        }
        echo json_encode($output);
    }

    function getRapidHivId($rapidHiv_id) {
        $sQuery = "SELECT *
                   FROM rapidhiv           
                   WHERE rapidHiv_id = '{$rapidHiv_id}'
                  ";
	...
	function addRapidHiv($data){
		if($this->db->insert('rapidhiv', $data)){
			return true; 
		}else{
			return false;
		}
	...
	function updateRapidHiv($data){
		extract($data);
		$this->db->where('rapidHiv_id', $rapidHiv_id);
		if($this->db->update('rapidhiv', $data)){
			return true;
		}else{
			return false;
		}
	...
	function deleteRapidHiv($data){
		extract($data);
		$insert = array(
			'rapidHiv_id' => $rapidHiv_id,
			'userName' => $this->session->userdata('userName'),
			'status' => 'deleted',
		);
		$this->db->where('rapidHiv_id', $rapidHiv_id );	
		if($this->db->update('rapidhiv', $insert)){
			$this->db->delete('rapidhiv', array('rapidHiv_id' => $data['rapidHiv_id']));
			return true;
		}else{
			return false;
		}
