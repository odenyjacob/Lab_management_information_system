...
function convetResultsetToArray($Q) {
	...
	function getBcTable() {
        ...
        $aColumns = array('bc_id', 'year', 'cycle', 'userName', 'parameter', 'results', 'status', 'target', 'timeStamp');

        //this array result column database
        $aResultColumns = array('bc_id','year', 'cycle', 'parameter', 'userName', 'results', 'status', 'target', 'timeStamp');
	...
	";
	  ...
	  ";
        ...
        for ($i = 0; $i < count($rResult); $i++) {
            $row = array();
            $row = $rResult[$i];
            $row['insights'] = "<a class='various' data-fancybox-type='iframe' href='" . base_url() . "index.php/bc/bc_graph/{$row['year']}'><i class='zoom fa fa-line-chart'>{$row['year']}<i class='zoom fa fa-bar-chart'> </a>";
            $row['edit'] = "<a class='various' data-fancybox-type='iframe' href='" . base_url() . "index.php/bc/popupadd/{$row['bc_id']}'>edit</a>";
            $row['delete'] = "<a class='deleteRecord' onclick= 'deleteRecord({$row['bc_id']})' >delete</a>";
            $output['aaData'][] = $row;
        }
        echo json_encode($output);
    }

    function getbcId($bc_id) {
        $sQuery = "SELECT *
                   FROM bc           
                   WHERE bc_id = '{$bc_id}'
                  ";
	...
	function deleteBc($data){
		...
		$this->db->where('bc_id', $bc_id );	
		if($this->db->update('bc', $insert)){
			$this->db->delete('bc', array('bc_id' => $data['bc_id']));
			return true;
		}else{
			return false;
		}
