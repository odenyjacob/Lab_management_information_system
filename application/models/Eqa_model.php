...
function convetResultsetToArray($Q) {
	...
	function updateEqa($data){
		extract($data);
		$this->db->where('eqa_id', $eqa_id);
		if($this->db->update('eqa', $data)){
			return true;
		}else{
			return false;
		}
	...
	function getEqaTable() {
        ...
        $aColumns = array('eqa_id', 'year', 'cycle', 'userName', 'parameter', 'results', 'status', 'target', 'timeStamp'); // These are are sorting and filtering columns - O'Brian Otieno
        
        $aResultColumns = array('eqa_id','year', 'cycle', 'parameter', 'userName', 'results', 'status', 'target', 'timeStamp'); //This array retuns result column from database O'Brian Otieno
        
        $sIndexColumn = "eqa_id"; /* Indexed column - for cardinality - O'Brian Otieno*/
       
        $sTable = "eqa";  //Specify the Database Table to use - O'Brian Otieno.
