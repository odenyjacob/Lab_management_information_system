...
class hematology_Model extends CI_Model {
...
function convetResultsetToArray($Q) {
	...
	function getHematologyTable() {
        ...
        $aColumns = array('hematology_id', 'year', 'cycle', 'userName', 'parameter', 'results', 'status', 'target', 'timeStamp');

        //this array result column database
        $aResultColumns = array('hematology_id','year', 'cycle', 'parameter', 'userName', 'results', 'status', 'target', 'timeStamp');

        /* Indexed column (used for fast and accurate table cardinality) */
        $sIndexColumn = "hematology_id";
	...
	";
	  ...
	  ";
        ...
        for ($i = 0; $i < count($rResult); $i++) {
            $row = array();
            $row = $rResult[$i];
            $row['insights'] = "<a class='various' data-fancybox-type='iframe' href='" . base_url() . "index.php/hematology/hematology_graph/{$row['year']}'><i class='zoom fa fa-line-chart'>{$row['year']}<i class='zoom fa fa-bar-chart'></a>";
            $row['edit'] = "<a class='various' data-fancybox-type='iframe' href='" . base_url() . "index.php/hematology/popupadd/{$row['hematology_id']}'>edit</a>";
            $row['delete'] = "<a class='deleteRecord' onclick= 'deleteRecord({$row['hematology_id']})' >delete</a>";
            $output['aaData'][] = $row;
        }
        echo json_encode($output);
    }

    function getHematologyId($hematology_id) {
        $sQuery = "SELECT *
                   FROM hematology           
                   WHERE hematology_id = '{$hematology_id}'
                  ";
	...
	function addHematology($data){
		if($this->db->insert('hematology', $data)){
			return true;
		}else{
			return false;
		}
	...
	function updateHematology($data){
		extract($data);
		$this->db->where('hematology_id', $hematology_id);
		if($this->db->update('hematology', $data)){
			return true;
		}else{
			return false;
		}
	...
	function deleteHematology($data){
		extract($data);
		$insert = array(
			'hematology_id' => $hematology_id,
			'userName' => $this->session->userdata('userName'),
			'status' => 'deleted',
		);
		$this->db->where('hematology_id', $hematology_id );	
		if($this->db->update('hematology', $insert)){
			$this->db->delete('hematology', array('hematology_id' => $data['hematology_id']));
			return true;
		}else{
			return false;
		}
