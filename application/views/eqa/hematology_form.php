...
<body>
			...
			<div class="col-md-12 col-md-offset-0">				 
				<div class="<?php echo $this->session->userdata('panel'); ?>">
			  		<div class="panel-heading">
			    		<h3 class="panel-title" align="center"><?= $this->hematology_id != NULL ? 'EDIT EQA HEMATOLOGY' : 'ADD NEW EQA HEMATOLOGY'; ?></h3>  
			  		</div>
			  		<div class="panel-body"> 
		    <h1>EQA Hematology Form</h1> <hr />
    <div id="body">

			<?php 
						  $attributes = array('class' => 'form-horizontal row-border', 'id' => 'hematology_form'); 
						 echo form_open('hematology/submitHematology', $attributes) ; 
						 ?>
						 ...
						 <div id="spinner" class="spinner" style="display:none; height: 200px; width: 200px;">
						    ...
						    <input type="hidden" name="type" id="type" value="<?= $this->hematology_id != NULL ? 'updateHematology' : 'addHematology'; ?>" />
						    <input type="hidden" name="hematology_id" id="hematology_id" value="<?= $this->hematology_id; ?>" />
							...
							<label>Parameter</label>
						 	<select name="parameter" id="parameter" class="form-control validate[required]">
						 		...
						 		<option value="Lymphs Absolute">Lymphs Absolute</option>
						 		<option value="MCV">MCV</option>
						 		<option value="Monocytes %">Monocytes %</option>
						 		<option value="Monocytes Absolute">Monocytes Absolute</option>
						 		<option value="Neut/Gran %">Neut/Gran %</option>
						 		<option value="Neut/Gran Absolute">Neut/Gran Absolute</option>
    ...
    $(document).ready(function() {
            ...
            $("#hematology_form").validationEngine();
        });
       ...
       // menuSelect("menuBrands");
        ...
        function save() {
            ...
            var validate =  $('#hematology_form').validationEngine("validate");
            if(!validate){
                return false;
            }
            ...
            var type = $("#type").val();
            var hematology_id = $("#hematology_id").val();
            var year = $("#year").val();
           ...
           $.ajax({
			type: "POST",
			url: "<?php echo base_url(); ?>" + "index.php/hematology/submitHematology",
			dataType: 'json',
			data: {type: type, year: year, cycle: cycle, results: results,
					parameter: parameter, target: target, hematology_id: hematology_id, 
					},
			beforeSend: function() {
				$("#spinner").show();
             },
