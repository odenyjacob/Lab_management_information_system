...
<body>
			...
			<div class="col-md-12 col-md-offset-0">				 
				<div class="<?php echo $this->session->userdata('panel'); ?>">
			  		<div class="panel-heading">
			    		<h3 class="panel-title" align="center"><?= $this->urinalysis_id != NULL ? 'EDIT EQA CHEMISTRY' : 'ADD NEW EQA CHEMISTRY'; ?></h3>  
			  		</div>
			  		<div class="panel-body"> 
		    <h1>EQA Urinalysis Form</h1> <hr />
    <div id="body">

			<?php 
						  $attributes = array('class' => 'form-horizontal row-border', 'id' => 'urinalysis_form'); 
						 echo form_open('urinalysis/submitUrinalysis', $attributes) ; 
						 ?>
						 ...
						 <div id="spinner" class="spinner" style="display:none; height: 200px; width: 200px;">
						    ...
						    <input type="hidden" name="type" id="type" value="<?= $this->urinalysis_id != NULL ? 'updateUrinalysis' : 'addUrinalysis'; ?>" />
						    <input type="hidden" name="urinalysis_id" id="urinalysis_id" value="<?= $this->urinalysis_id; ?>" />
							...
							<label>Parameter</label>
						 	<select name="parameter" id="parameter" class="form-control validate[required]">
						 		...
						 		<option value="Blood/Hgb, Urine">Blood/Hgb, Urine</option>
						 		<option value="Bilirubin, Urine">Bilirubin, Urine</option>
						 		<option value="Glucose, urine">Glucose, urine</option>
						 		<option value="hCG, urine">hCG, urine</option>
						 		<option value="Ketones, urine">Ketones, urine</option>
						 		<option value="KOH prep">KOH prep</option>
						 		<option value="Leuko Esterase, urine">Leuko Esterase, urine</option>
						 		<option value="pH, urine">pH, urine</option>
						 		<option value="Nitrite, urine">Nitrite, urine</option>
						 		<option value="Protein, urine, qual">Protein, urine, qual</option>
						 		<option value="Spec Gravity, urine">Spec Gravity, urine</option>
						 		<option value="Urine Sediment ID">Urine Sediment ID</option>
						 		<option value="Urobilinogen, urine">Urobilinogen, urine</option>
						 		<option value="Vaginal Wet prep">Vaginal Wet prep</option>
						 		<option value="Yeast/Clue Cells, Vaginal">Yeast/Clue Cells, Vaginal</option>
						 	</select>
						 <?php 
							/* Rejected Samples
    ...
    $(document).ready(function() {
            ...
            $("#urinalysis_form").validationEngine();
        });
       ...
       // menuSelect("menuBrands");
        ...
        function save() {
            ...
            var validate =  $('#urinalysis_form').validationEngine("validate");
            if(!validate){
                return false;
            }
            ...
            var type = $("#type").val();
            var urinalysis_id = $("#urinalysis_id").val();
            var year = $("#year").val();
           ...
           $.ajax({
			type: "POST",
			url: "<?php echo base_url(); ?>" + "index.php/urinalysis/submitUrinalysis",
			dataType: 'json',
			data: {type: type, year: year, cycle: cycle, results: results,
					parameter: parameter, target: target, urinalysis_id: urinalysis_id, 
					},
			beforeSend: function() {
				$("#spinner").show();
             },
