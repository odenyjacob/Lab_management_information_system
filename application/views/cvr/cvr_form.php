			...
			<div class="col-md-12 col-md-offset-0">				
				<div class="<?php echo $this->session->userdata('panel'); ?>">
			  		<div class="panel-heading">
			    		<h3 class="panel-title" align="center"><?= $this->cvr_id != NULL ? 'EDIT CRITICAL VALUE REPORTING' : 'ADD NEW CRITICAL VALUE'; ?></h3>  
			  		</div>
			  		<div class="panel-body"> 
		    <h1>Critical Value Reporting Form</h1> <hr />
    <div id="body">
			...
			<?php 
						  $attributes = array('class' => 'form-horizontal row-border', 'id' => 'cvrForm'); 
						 echo form_open('cvr/submitCvr', $attributes) ; 
						 ?>
						 ...
						 <div id="spinner" class="spinner" style="display:none; height: 200px; width: 200px;">
						    ...
						    <input type="hidden" name="type" id="type" value="<?= $this->cvr_id != NULL ? 'updateCvr' : 'addCvr'; ?>" />
						    <input type="hidden" name="cvr_id" id="cvr_id" value="<?= $this->cvr_id; ?>" />
						 ...
						 <?php 
							// Number of critical values reported.
							echo form_label('Number of critical values reported');
							$data_name = array(
							'type' => 'number',
							'name' => 'reported',
							'class' => 'form-control validate[required]',
							'placeholder' => 'Enter Critical Values Reported',
							'id' => 'reported',
							'required' => 'required',
							'value' => $this->reported,
							//'autofocus' => true,
							...
							// Middle Name Field
							echo form_label('Total number of critical value events');
							$data_name = array(
							'type' => 'number',
							'name' => 'all_events',
							'class' => 'form-control validate[required]',
							'placeholder' => 'Total number of critical value events',
							'id' => 'all_events',
							'value' => $this->all_events,
							);
							echo form_input($data_name); 
							echo br();
    ...
    $(document).ready(function() {
       ...
       // menuSelect("menuBrands");
        ...
        function save() {
            ...
            var validate =  $('#cvrForm').validationEngine("validate");
            if(!validate){
                return false;
            }
           ...
           $.ajax({
			type: "POST",
			url: "<?php echo base_url(); ?>" + "index.php/cvr/submitCvr",
			dataType: 'json',
			data: {type: type, year: year, month: month, reported: reported, 
					all_events: all_events, target: target, cvr_id: cvr_id, 
					},
			beforeSend: function() {
				$("#spinner").show();
             },
