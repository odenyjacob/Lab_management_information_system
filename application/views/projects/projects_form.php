...
<body>
			...
			<div class="col-md-12 col-md-offset-0">				
				<div class="<?php echo $this->session->userdata('panel'); ?>">
			  		<div class="panel-heading">
			    		<h3 class="panel-title" align="center"><?= $this->id != NULL ? 'EDIT PROJECT' : 'ADD PROJECT'; ?></h3>  
			  		</div>
    ...
    <div id="body">
			...
			<?php 
				$attributes = array('class' => 'form-horizontal row-border', 'id' => 'projectsForm'); 
				echo form_open('Projects/submitProjects', $attributes) ; 
			?>
						 ...
						 <div id="spinner" class="spinner" style="display:none; height: 200px; width: 200px;">
						    ...
						    <input type="hidden" name="type" id="type" value="<?= $this->id != NULL ? 'updateProjects' : 'addProject'; ?>" />
						    <input type="hidden" name="id" id="id" value="<?= $this->id; ?>" />
						 	<label>Branch</label>
						 	<select name="branch" id="branch" class="form-control validate[required]">
						 		<option value="<?= $this->branch ?>"><?= $this->branch != NULL ? $this->branch : 'Select Branch' ?> </option>
    ...
    $(document).ready(function() {
            ...
            $("#projectsForm").validationEngine(); 
        });
        
        function save() {
        	var validate =  $('#projectsForm').validationEngine("validate");
            if(!validate){
                return false;
            }
            ...
            var project = $("#project").val(); 
	        	 ...
	        	 $.ajax({
					type: "POST",
					url: "<?php echo base_url(); ?>" + "index.php/projects/submitProjects",
					dataType: 'json',
					data: {type: type, id: id, branch: branch, project:project },  
					beforeSend: function() {
						$("#spinner").show();
		             },
