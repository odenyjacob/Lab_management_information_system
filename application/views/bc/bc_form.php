...
<body>
			...
			<div class="col-md-12 col-md-offset-0">				 
				<div class="<?php echo $this->session->userdata('panel'); ?>">
			  		<div class="panel-heading">
			    		<h3 class="panel-title" align="center"><?= $this->bc_id != NULL ? 'EDIT BLOOD CELL, PARASITE & CTNG' : 'ADD NEW BLOOD CELL, PARASITE & CTNG'; ?></h3>  
			  		</div>
    ...
    <div id="body">
			...
			<?php 
						 ...
						 echo form_open('bc/submitBc', $attributes) ; 
						 ?>
						 ...
						 <div id="spinner" class="spinner" style="display:none; height: 200px; width: 200px;">
						    ...
						    <input type="hidden" name="type" id="type" value="<?= $this->bc_id != NULL ? 'updateBc' : 'addBc'; ?>" />
						    <input type="hidden" name="bc_id" id="bc_id" value="<?= $this->bc_id; ?>" />
							...
							<label>Parameter</label>
						 	<select name="parameter" id="parameter" class="form-control validate[required]">
						 		...
						 		<option value="Blood Cell ID">Blood Cell ID</option>
						 		<option value="Blood Parasite ID">Blood Parasite ID</option>
						 		<option value="Chlamydia trachomatis, NAA">Chlamydia trachomatis, NAA</option>
						 		<option value="Neisseria gonorrhoeae, NAA">Neisseria gonorrhoeae, NAA</option>
						 	</select>
						 <?php 
    ...
    $(document).ready(function() {
       ...
       // menuSelect("menuBrands");
        ...
        function save() {
           ...
           $.ajax({
			type: "POST",
			url: "<?php echo base_url(); ?>" + "index.php/bc/submitBc",
			dataType: 'json',
			data: {type: type, year: year, cycle: cycle, results: results,
					parameter: parameter, target: target, bc_id: bc_id, 
					},
			beforeSend: function() {
				$("#spinner").show();
             },
