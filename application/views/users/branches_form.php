...
<body>
			...
			<div class="col-md-12 col-md-offset-0">				
				<div class="<?php echo $this->session->userdata('panel'); ?>">
			  		<div class="panel-heading">
			    		<h3 class="panel-title" align="center"><?= $this->id != NULL ? 'EDIT BRANCH' : 'ADD NEW BRANCH'; ?></h3>  
			  		</div>
			  		<div class="panel-body"> 
		    <h1>Branchs Form</h1> <hr />
    <div id="body">
 
			<?php 
				$attributes = array('class' => 'form-horizontal row-border', 'id' => 'branchesForm'); 
				echo form_open('centers/submitBranch', $attributes) ; 
			?>
						 ...
						 <div id="spinner" class="spinner" style="display:none; height: 200px; width: 200px;">
						    ...
						    <input type="hidden" name="type" id="type" value="<?= $this->id != NULL ? 'updateBranch' : 'addBranch'; ?>" />
						    <input type="hidden" name="id" id="id" value="<?= $this->id; ?>" />
							<label>Center</label>
						 	<select name="center" id="center" class="form-control validate[required]">
						 		<option value="<?= $this->center ?>"><?= $this->center != NULL ? $this->center : 'Select Center' ?> </option>
						 ...
						 <?php 
							...
							echo br(2);
			  				...
			  				echo form_submit('submit', 'Submit Branch', "class=' btn btn-lg btn-block $buttonclass' id='submit'");
							echo form_close();
						?>
    ...
    $(document).ready(function() {
            ...
            $("#branchesForm").validationEngine(); 
        });
        
        function save() {
        	var validate =  $('#branchesForm').validationEngine("validate");
            if(!validate){
                return false;
            }
            ...
            var branch = $("#branch").val();  
	        	 ...
	        	 $.ajax({
					type: "POST",
					url: "<?php echo base_url(); ?>" + "index.php/centers/submitBranch",
					dataType: 'json',
					data: {type: type, id: id, center: center, branch: branch },  
					beforeSend: function() {
						$("#spinner").show();
		             },
