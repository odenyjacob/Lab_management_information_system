...
<script>
	$(document).ready(function(){		
		$(".approve").click(function(event){
			...
			$.ajax({
				type: "POST",
				url: "<?php echo base_url(); ?>" + "index.php/fms/approveMyImprestApprovals",
				dataType: 'json',
				data: {
					impNumber: impNumber,	
				},
				success: function (response){
					if(response){
					alert("#" + response.impNumber + ": Imprest Request Approved Successfully!");
					}else{
		...
		$(".reject").click(function(event){
			...
			$.ajax({
				type: "POST",
				url: "<?php echo base_url(); ?>" + "index.php/fms/rejectMyImprestApprovals",
				dataType: 'json',
				data: {
					impNumber: impNumber,	
				},
				success: function (response){
					if(response){
					alert("#" + response.impNumber + ": Imprest Request Rejected Successfully!");
					}else{
						alert("Something went wrong while processing. Try again later");
					}
				}
			...
			<div class="col-md-12 col-md-offset-0">				
				<div class="<?php echo $this->session->userdata('panel'); ?>">
			  		<div class="panel-heading">
			    		<h3 class="panel-title" align="center"> :::: MY IMPREST APPROVALS :::</h3>  
			  		</div>
			  		<div class="panel-body" style="overflow-y: scroll; min-height: 10;"> 
						...
						echo "<thead>
				        	...
				        	<tr> <td>{$row->impNumber}</td> <td> {$row->FName} {$row->LName}</td>  <td> {$row->amount} </td>   <td> {$row->payee_account} </td>  <td>{$row->date_raised} </td> <td> {$row->purpose} </td>  <td> {$row->remarks} </td>  <td> {$row->supervisor_approval} </td>  <td><center>"; if($row->supervisor_approval==="Pending Approval"){echo " <button class='approve btn btn-sm  $buttonclass'   value='{$row->impNumber}'>Approve</button> '<br /> <button class='reject btn btn-sm  btn-warning'   value='$row->impNumber'>Reject</button>";} echo " </center>  </td>	 </tr>
				        	";}else{
