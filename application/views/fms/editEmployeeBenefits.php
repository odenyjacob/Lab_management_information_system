...
<script>
	$(document).ready(function(){
	  ...
	  $("#empNumber").keyup(function(){
		...
		$.ajax({
			type: "post",
			url: "<?php echo base_url(); ?>" + "index.php/fms/editEb", 
			cache: false,				
					...
					try{
						...
						$.each(obj, function(i,val){									
						   // items.push($('LName').text(val.empNumber + "- " + val.FName + "-" + val.LName));
						    ...
						    items.push($('#basic_salary').val(val.basic_salary ));
						    items.push($('#house_allowance').val(val.house_allowance ));
						    items.push($('#medical_allowance').val(val.medical_allowance ));
						    items.push($('#over_time').val(val.over_time ));
						    items.push($('#travel_reimbursement').val(val.travel_reimbursement ));
						    items.push($('#arrears').val(val.arrears ));
						    items.push($('#supplement1').val(val.supplement1 ));
						    items.push($('#supplement2').val(val.supplement2 ));
						    items.push($('#supplement3').val(val.supplement3 ));
						    items.push($('#id').val(val.id ));
						});	
		...
		$("#submit").click(function(event) {
			...
			jQuery.ajax({
				type: "POST",	
				url: "<?php echo base_url(); ?>" + "index.php/fms/pEditedEb",
				dataType: 'json',
				data: {
					empNumber: empNumber,
			...
			<div class="col-md-12 col-md-offset-0">				
				<div class="<?php echo $this->session->userdata('panel'); ?>">
			  		<div class="panel-heading">
			    		<h3 class="panel-title" align="center"> :::: EDIT EMPLOYEE BENEFITS :::</h3>  
			  		</div>
			  		<div class="panel-body"> 
			  			<div id="form_input">
							...
							echo "<ul id='finalResult' style='decoration: none; color: red;'></ul>";
							// First Name Field
