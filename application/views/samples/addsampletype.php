...
<body>
			...
			<div class="col-md-12 col-md-offset-0">				
				<div class="<?php echo $this->session->userdata('panel'); ?>">
			  		<div class="panel-heading">
			    		<h3 class="panel-title" align="center"><?= $this->id != NULL ? 'EDIT SAMPLE TYPE' : 'ADD SAMPLE TYPE'; ?></h3>  
			  		</div>
			  		<div class="panel-body"> 
		    <h1>Add Sample Type</h1> <hr />
    <div id="body">
 
			<?php 
				$attributes = array('class' => 'form-horizontal row-border', 'id' => 'samplesTypeForm'); 
				echo form_open('centers/submitSampleType', $attributes) ; 
			?>
						 ...
						 <div id="spinner" class="spinner" style="display:none; height: 200px; width: 200px;">
						    ...
						    <input type="hidden" name="type" id="type" value="<?= $this->id != NULL ? 'updateSampleType' : 'addSampleType'; ?>" />
						    <input type="hidden" name="id" id="id" value="<?= $this->id; ?>" />
						 
						 <?php 
							// Sample Type
							echo form_label('Sample Type');
							$data_name = array(
							'type' => 'text',
							'name' => 'sample_type',
			  			...
			  			echo form_submit('submit', 'Add Sample Type', "class=' btn btn-lg btn-block $buttonclass' id='submit'");
						echo form_close();
						?>
    ...
    $(document).ready(function() {
            ...
            $("#samplesTypeForm").validationEngine(); 
        });
        
        function save() {
        	var validate =  $('#samplesTypeForm').validationEngine("validate");
            if(!validate){
                return false;
            }
            ...
            var sample_type = $("#sample_type").val();  
	        	 ...
	        	 $.ajax({
					type: "POST",
					url: "<?php echo base_url(); ?>" + "index.php/samples/submitSampleType",
					dataType: 'json',
					data: {type: type, id: id, sample_type: sample_type},  
					beforeSend: function() {
						$("#spinner").show();
		             },
